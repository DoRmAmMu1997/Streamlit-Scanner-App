...
- AI verdict still malformed after the bounded retry → the AI screener emits an `error` `AIEvaluationRecord` (persisted for audit) **and** a `phase="ai_validation"` compute failure; the run goes `PARTIAL` (some) / `FAILED` (all), `ai_validation_failures` is reported, and no result row is produced.
