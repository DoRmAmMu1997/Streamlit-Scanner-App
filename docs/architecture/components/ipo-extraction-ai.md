...
| Terminal SDK status wins over output text | Rejected rate/billing events, `ResultMessage.is_error`, missing CLI, and failed CLI processes become typed errors before JSON parsing; a failed run cannot enqueue a proposal even when its text validates. |
