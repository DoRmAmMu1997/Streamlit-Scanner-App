...
| **Per-screener provenance (PROV-002)** | Every `compute_signal` returns a `provenance` receipt via `self.build_provenance(triggered_rules=…, indicator_values=…, source=…)` — deterministic screeners use `source="deterministic"`; the two AI screeners use `source="hybrid"` with an `ai` receipt. `build_result_frame` validates it (strict contract) and the persistence layer expands it into `provenance_json`. See [scan-service-and-provenance.md](scan-service-and-provenance.md). |
