...
| `SerpApiClient(api_key=None, session=None)` · `.search(query, max_results=5)` | Fixed `ENDPOINT`; `SerpApiSetupError` plus the `SerpApiSearchError` family — IPO-012 added `SerpApiQuotaError`, `SerpApiRateLimitError` and `SerpApiAuthError` as **subclasses** (so `except SerpApiSearchError` still catches everything), each carrying `status_code`. A query Google returned nothing for now yields `[]` instead of raising. India-localized (`gl=in,hl=en`). |
