...
### `comparison_page.py`
`_render_comparison_page()` (the view) · pure helpers `_comparison_screener_options(groups)`, `_comparison_universe_options(groups, screener_key)`, `_comparison_export_csv(frame)`, `_section_frame(rows)`, `_decimal_value(value)`. It reads options through `list_finalized_scan_groups()` and comparison rows through `build_scan_comparison()`.
