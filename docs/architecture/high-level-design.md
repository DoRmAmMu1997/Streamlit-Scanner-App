...
- **CI quality gates** (`.github/workflows/quality-and-security.yml`, Python 3.11): `pytest` (coverage ≥84% on `backend`/`screeners`/`ui`), `compileall`, `ruff`, `bandit`, `pip-audit`, `pre-commit`, plus golden-snapshot + Alembic drift tests.
