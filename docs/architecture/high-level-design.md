...
- **Roadmap (backlog)**: RANK-* (`final_score` ranking), verified benchmark instrument IDs (VALID-002B), richer validation visualizations beyond v1 tables, AUTH-003 (role-gated features), and later DEPLOY-* hosting automation. These land in reserved columns / JSON or runtime docs without flag-day migrations.
