...
| Forward-return validation | [validation](components/validation.md) | VALID-002 calculator/service, VALID-003A aggregate metrics for `signal_forward_returns` rows, and the VALID-003B read-only Validation / Signal Performance dashboard |
...
- **Persistence, provenance, and validation** — every shortlisted row carries a deterministic receipt (PROV-002: `triggered_rules` + `indicator_values` + `source`, built by `BaseScanner.build_provenance`); AI screeners add a tamper-evident verdict receipt (PROV-003: model, semantic prompt version, prompt/evidence/context SHA-256, sanitized source URLs — never raw scraped/model text) persisted to the `ai_evaluations` ledger. VALID-002 computes per-signal forward returns into `signal_forward_returns` without re-running the screener, VALID-003A aggregates those stored rows into screener/universe/horizon performance metrics, and VALID-003B surfaces them in a read-only Validation / Signal Performance dashboard. ([scan-service-and-provenance](components/scan-service-and-provenance.md), [storage-persistence](components/storage-persistence.md), [validation](components/validation.md))
...
| **Validation aggregates stay read-only (VALID-003A)** | Screener-level validation metrics read existing `signal_forward_returns` rows through repository-owned joins. Pending and insufficient rows remain separate counts, hit rate is computed only from stored computed returns, and missing benchmark/excess values stay null rather than being fabricated. |
