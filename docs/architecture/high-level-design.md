...
- **Render managed hosting (DEPLOY-003)**: `render.yaml` Blueprint reusing the same image — a `scanner-web` web service (persistent disk at `DATA_DIR` for the candle cache) + a managed `scanner-db` Postgres auto-wired into `DATABASE_URL` + an ephemeral `scanner-daily-scan` cron. Render disks are single-attach, so the disk lives on the web service while the cron re-fetches candles and writes to the shared Postgres. All secrets are dashboard-provided (`sync: false`).
