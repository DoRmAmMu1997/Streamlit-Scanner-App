...
| **Current authority and terminal SDK status govern AI execution** | Retained browser state never restores a demoted user's `RUN_SCAN` permission; all four MCP agents disable SDK built-ins explicitly; and failed IPO SDK/CLI outcomes become typed receipts before model text can be parsed or stored. See [AI execution boundaries](ai-execution-boundaries.md). |
