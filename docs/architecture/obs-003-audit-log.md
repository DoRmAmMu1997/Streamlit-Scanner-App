...
returns `False`, mirroring how scan persistence is best-effort. On a fresh checkout
whose database is not yet migrated, the durable row is simply skipped.
...
exactly once per browser session. Button/download/form events are edge-triggered
and need no dedup.
...
| `data_refresh_started` | `app.py` `prefetch_data_assets()` | System event (`user_email=NULL`); best-effort. |
...
- **`data_refresh_started` is the only system event.** It is recorded from the CLI
  prefetch path; on a brand-new checkout (no migrated DB) the best-effort write is
  skipped — that is expected, not a bug.
- **Dedup is per session, not global.** Two browser sessions for the same user each
  record one `login_success`. That is intended (a session = a sign-in).
