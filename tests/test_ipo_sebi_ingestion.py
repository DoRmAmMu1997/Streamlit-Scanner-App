...
def _filing(
    *,
    ...
    """Build the reusable filing fixture used by the scenarios below."""
...
def test_ingest_creates_issue_document_and_is_repeat_run_idempotent(
    ...
    """Pin ingest creates issue document and is repeat run idempotent as an executable IPO regression contract."""
    filing = _filing()
...
def test_later_documents_advance_status_but_older_filings_never_regress_it(
    ...
    """Advance status with newer filing stages while never regressing lifecycle."""
...
def test_single_case_insensitive_legacy_company_is_claimed_once(file_session_factory) -> None:
    """Pin single case insensitive legacy company is claimed once as an executable IPO regression contract."""
    legacy = create_issue(
        IpoIssueData(
...
def test_cross_issue_fingerprint_or_url_conflict_rolls_back_whole_batch(
    ...
    """Pin cross issue fingerprint or url conflict rolls back whole batch as an executable IPO regression contract."""
    owner = create_issue(
        IpoIssueData(
...
def test_same_issue_url_match_claims_missing_fingerprint_and_updates_metadata(
    ...
    """Claim a missing fingerprint when the canonical URL already owns the row."""
    issue = create_issue(
        IpoIssueData(
            company_name="Example Limited",
            issue_type=IpoIssueType.UNKNOWN,
...
def test_latest_filing_date_is_the_global_ingestion_watermark(file_session_factory) -> None:
    """Pin latest filing date is the global ingestion watermark as an executable IPO regression contract."""
